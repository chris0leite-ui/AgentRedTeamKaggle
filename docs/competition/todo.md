...
## Next
- [ ] **Full lab measurement (unblocks sizing):** run `lab/` (20 trials × **both** models) — smoke
  gave gpt_oss (100% @ 5.3s → ~1523), but we still need **gemma** (bigger → likely the binding
  constraint) and a larger N to trust the fire-rate. (See `experiments.md` E2, `iteration.md`.)
